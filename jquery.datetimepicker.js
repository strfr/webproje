...
(function( $ ){
	$.fn.datetimepicker = function( opt ){
		...
		var createDateTimePicker = function( input ){
			var datetimepicker = $('<div class="datetimepicker"></div>'),
				copyright = $('<div class="copyright"><a target="_blank" href="http://xdsoft.net/jqplugins/datetimepicker/">xdsoft.net</a></div>'),
				datepicker = $('<div class="datepicker active"></div>'),
				mounth_picker = $('<div class="mounthpicker"><button type="button" class="prev"></button><div class="label month"></div><div class="label year"></div><button type="button" class="next"></button></div>'),
				calendar = $('<div class="calendar"></div>'),
				timepicker = $('<div class="timepicker active"><button type="button" class="prev"></button><div class="time_box"></div><button type="button" class="next"></button></div>'),
			...
			timepicker.find('.time_box').append(scrollbar);
			(function(){
				var move = 0;
			...
			timepicker.find('.time_box').append(timebox);
			...
			var datetime = function(){
				...
				self.nextMonth = function(){
					...
					options.onChangeMonth&&options.onChangeMonth.call&&options.onChangeMonth.call(datetimepicker,datetimepicker.data('datetime').currentTime,datetimepicker.data('input'));
					datetimepicker.trigger('change.xdsoft');
					return month;
				};
			...
			mounth_picker.find('.prev,.next').on('mousedown.xdsoft',function(){
				var $this = $(this), timer = 0, stop = false;
				(function(v){
					var month =  datetimepicker.data('datetime').currentTime.getMonth();
					if( $this.hasClass( options.next ) ){
						datetimepicker.data('datetime').nextMonth();
					}else if( $this.hasClass( options.prev ) ){
						datetimepicker.data('datetime').prevMonth();
					}
			...
			timepicker.find('.prev,.next').on('mousedown.xdsoft',function(){
			...
			datetimepicker.on('change.xdsoft',function(){
				...
				while( i<xd.currentTime.getDaysInMonth()||start.getDay()!=options.dayOfWeekStart||xd.currentTime.getMonth()==start.getMonth() ){
					...
					table+='<td data-date="'+start.getDate()+'" data-month="'+start.getMonth()+'" data-year="'+start.getFullYear()+'"'+
						' class="'+
							...
							?'disabled ':' ')+
							(xd.currentTime.getMonth()!=start.getMonth()?' other_month ':' ')+
							(xd.currentTime.dateFormat('d.m.Y')==start.dateFormat('d.m.Y')?' current ':' ')+
							(today.dateFormat('d.m.Y')==start.dateFormat('d.m.Y')?' today ':' ')
				...
				table+='</tbody></table>';
				calendar.html(table);
				mounth_picker.find('.label').eq(0).text(options.i18n[options.lang].months[xd.currentTime.getMonth()]);
				mounth_picker.find('.label').eq(1).text(xd.currentTime.getFullYear());
				var time = '',h = '',m ='';
				var line_time = function (h,m){
					...
					time+= '<div class="'+
								...
								((parseInt(today.getHours())==parseInt(h)&&parseInt(today.getMinutes())==parseInt(m))?' today ':'')+
				...
				timebox.html(time);
				//timebox.find('.current').length&&timebox.css('marginTop','-'+parseInt(timebox.find('.current').index()*options.timeHeightInTimePicker)+'px');
			});
			datetimepicker.on('open.xdsoft',function(){
				if( timebox.find('.current').length ){
					var pheight = timeboxparent[0].offsetHeight-2,
						height = timebox[0].offsetHeight,
						top = timebox.find('.current').index()*options.timeHeightInTimePicker+1;
			...
			calendar.on('mousedown.xdsoft','td',function(){
				if($(this).hasClass('disabled'))
					return false;
				var ct = datetimepicker.data('datetime').currentTime;
				ct.setFullYear($(this).data('year'));
				ct.setMonth($(this).data('month'));
				ct.setDate($(this).data('date'));
				datetimepicker.trigger('select.xdsoft',[ct]);
				input.val( datetimepicker.data('datetime').str() );
				if( (options.closeOnDateSelect===true||( options.closeOnDateSelect===0&&!options.timepicker ))&&!options.inline )
					datetimepicker.trigger('close.xdsoft');
				options.onSelectDate&&options.onSelectDate.call&&options.onSelectDate.call(datetimepicker,datetimepicker.data('datetime').currentTime,datetimepicker.data('input'));
				datetimepicker.trigger('change.xdsoft');
			...
			timebox.on('mousedown.xdsoft','div',function(){
				...
				datetimepicker.trigger('select.xdsoft',[ct]);
				datetimepicker.data('input').val( datetimepicker.data('datetime').str() );
				!options.inline&&datetimepicker.trigger('close.xdsoft');
				options.onSelectTime&&options.onSelectTime.call&&options.onSelectTime.call(datetimepicker,datetimepicker.data('datetime').currentTime,datetimepicker.data('input'));
				datetimepicker.trigger('change.xdsoft');
				datetimepicker.trigger('changedatetime.xdsoft');
			});
			...
			datetimepicker.on('changedatetime.xdsoft',function(){
				options.onChangeDateTime&&options.onChangeDateTime.call&&options.onChangeDateTime.call(datetimepicker,datetimepicker.data('datetime').currentTime,datetimepicker.data('input'));
			});
			...
			input.mousewheel&&input.mousewheel(function( event, delta, deltaX, deltaY ){
				...
				if( !options.datepicker && options.timepicker ){
					...
					input.val&&input.val( datetimepicker.data('datetime').str() );
					datetimepicker.trigger('changedatetime.xdsoft');
					return false;
				}
			});
			datetimepicker.on('open.xdsoft',function(){
				var onShow = true;
				options.onShow&&options.onShow.call&&(onShow=options.onShow.call(datetimepicker,datetimepicker.data('datetime').currentTime,datetimepicker.data('input')));
				if( onShow!==false ){
					var setPos = function(){
			...
			datetimepicker.on( 'close.xdsoft',function(){
				var onClose = true;
				options.onClose&&options.onClose.call&&(onClose=options.onClose.call(datetimepicker,datetimepicker.data('datetime').currentTime,datetimepicker.data('input')));
			...
			var _datetime = new datetime,timer = 0,timer1 = 0;
			datetimepicker.data('datetime',_datetime);
			datetimepicker.setOptions(options);
			_datetime.setCurrentTime( options.value?options.value:(input&&input.val&&input.val())?input.val():new Date );
			input.data( 'datetimepicker',datetimepicker )
			 .on('open.xdsoft focusin.xdsoft mousedown.xdsoft',function(event){
				...
				timer = setTimeout(function(){
					if( input.is(':disabled')||input.is(':hidden')||!input.is(':visible') )
						return;
					_datetime.setCurrentTime((input&&input.val&&input.val())?input.val():new Date);
		...
		var destroyDateTimePicker = function(input){
			...
			if( datetimepicker ){
				var _datetime = datetimepicker.data('datetime');
				delete _datetime;
				datetimepicker.remove();
				delete datetimepicker;
				input.data( 'datetimepicker',null );
		...
		return this.each(function(){
			var datetimepicker;
			if( datetimepicker = $(this).data('datetimepicker') ){
				if( $.type(opt) === 'string' ){
					switch(opt){
						case 'show':
							$(this).select().focus();
					...
					$(this).data('datetimepicker').setOptions(options);
				}
				return 0;
			}else
